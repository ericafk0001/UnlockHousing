  ...
  try {
    ...
    if (typeof data === "string") {
      ...
      for (const line of lines) {
        ...
        yPosition +=
          (Array.isArray(splitText) ? splitText.length : 1) * lineHeight;
      }
      pdf.save(fileName);
      return true;
    }
    ...
    const addLeftRightLine = (
      ...
      pdf.setFont(
        "Helvetica",
        options.leftBold ? "bold" : options.leftItalic ? "italic" : "normal",
      );
    ...
    pdf.setFontSize(18);
    pdf.setFont("Helvetica", "bold");
    pdf.text(data.personalInfo.fullName || "Your Name", centerX, yPosition, {
      align: "center",
    });
    yPosition += 7;
    ...
    if (contactInfo.length > 0) {
      ...
      yPosition +=
        (Array.isArray(contactSplit) ? contactSplit.length : 1) * lineHeight;
      yPosition += 3;
    }
    ...
    if (data.education.length > 0) {
      ...
      data.education.forEach((edu) => {
        addLeftRightLine(edu.school, edu.graduationDate, {
          leftBold: true,
          fontSize: 10,
        });
    ...
    if (data.experience.length > 0) {
      ...
      data.experience.forEach((exp) => {
        ...
        addLeftRightLine(exp.company || " ", exp.location || "", {
          leftItalic: true,
          fontSize: 9.5,
        });
        addLeftRightLine(exp.jobTitle || " ", dateInfo.join(" - "), {
          leftBold: true,
          fontSize: 10,
        });
    ...
    if (data.leadershipCommunity.length > 0) {
      ...
      data.leadershipCommunity.forEach((lead) => {
        ...
        addLeftRightLine(lead.organization || " ", lead.location || "", {
          leftItalic: true,
          fontSize: 9.5,
        });
        addLeftRightLine(lead.position || " ", dateInfo.join(" - "), {
          leftBold: true,
          fontSize: 10,
        });
    ...
    // Criminal History
    if (data.criminalHistory.include && data.criminalHistory.details.trim()) {
      addHeader("CRIMINAL HISTORY");
      addText(data.criminalHistory.details.trim(), 9.5);
    }
