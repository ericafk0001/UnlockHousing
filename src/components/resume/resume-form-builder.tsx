...
type ActiveTab = 'personal' | 'education' | 'experience' | 'leadership' | 'skills' | 'honors';
...
export const ResumeFormBuilder: React.FC<ResumeFormBuilderProps> = ({ onDataChange }) => {
  ...
  const [achievementDrafts, setAchievementDrafts] = useState<Record<string, string>>({});
  const [aiLoadingByExperience, setAiLoadingByExperience] = useState<Record<string, boolean>>({});
  const [aiErrorByExperience, setAiErrorByExperience] = useState<Record<string, string>>({});
  ...
  const updateExperience = <K extends ExperienceField>(index: number, field: K, value: ResumeFormData['experience'][number][K]) => {
  ...
  const addAchievement = (index: number, achievement: string) => {
    if (achievement.trim()) {
      const updated = [...formData.experience];
      updated[index].achievements = [...updated[index].achievements, achievement.trim()];
      const formUpdate = { ...formData, experience: updated };
      setFormData(formUpdate);
      onDataChange(formUpdate);
    }
  ...
  const updateLeadership = <K extends LeadershipField>(index: number, field: K, value: ResumeFormData['leadershipCommunity'][number][K]) => {
  ...
  const enhanceAchievements = async (index: number, mode: 'rewrite' | 'generate') => {
    const experience = formData.experience[index];
    if (!experience) return;

    const contextText = achievementDrafts[experience.id] || '';
    if (!experience.jobTitle && !experience.company && !contextText && mode === 'generate') {
      setAiErrorByExperience(prev => ({
        ...prev,
        [experience.id]: 'Add at least a role, company, or notes so AI has context.',
      }));
      return;
    }
    ...
    try {
      ...
      const normalizedAchievements = Array.isArray(payload.achievements)
        ? payload.achievements
            ...
            .filter((item: string) => item.length > 0 && !item.includes('{"achievements"'))
            .slice(0, 5)
        : [];

      if (normalizedAchievements.length === 0) {
        throw new Error('AI response could not be parsed into clean bullet points. Try adding more notes and retry.');
      }
  ...
  const removeLeadershipAchievement = (leadIndex: number, achIndex: number) => {
    const updated = [...formData.leadershipCommunity];
    updated[leadIndex].achievements = updated[leadIndex].achievements.filter((_, i) => i !== achIndex);
    const formUpdate = { ...formData, leadershipCommunity: updated };
    setFormData(formUpdate);
    onDataChange(formUpdate);
  };

  const addSkill = (category: 'softSkills' | 'languages' | 'programming', skill: string) => {
  ...
  const removeSkill = (category: 'softSkills' | 'languages' | 'programming', skill: string) => {
    const updated = {
      ...formData,
      skills: {
        ...formData.skills,
        ...
        {activeTab === 'education' && (
          <div className="space-y-4">
            {formData.education.map((edu, index) => (
              <div key={edu.id} className="rounded-lg border border-gray-200 bg-gray-50">
                <div
                ...
                >
                  <div className="text-left">
                    <p className="font-medium text-gray-900">{edu.school || 'School'}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {expandedEducation === edu.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </div>
                </div>
        ...
        {activeTab === 'experience' && (
          <div className="space-y-4">
            {formData.experience.map((work, index) => (
              <div key={work.id} className="rounded-lg border border-gray-200 bg-gray-50">
                <div
                  onClick={() => setExpandedExperience(expandedExperience === work.id ? null : work.id)}
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-100 cursor-pointer"
                >
                  <div className="text-left">
                    <p className="font-medium text-gray-900">{work.jobTitle || 'Job Title'}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {expandedExperience === work.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </div>
                </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex gap-2">
                          <input
                            ...
                            onKeyPress={e => {
                              if (e.key === 'Enter') {
                                addAchievement(index, (e.target as HTMLInputElement).value);
                        ...
                        <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-2">
                          <label className="block text-xs font-semibold text-blue-900">AI Notes (optional)</label>
                          <textarea
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button
                              onClick={() => enhanceAchievements(index, 'generate')}
                            ...
                            >
                              {aiLoadingByExperience[work.id] ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />}
                              Generate Bullets
                            </button>
                            <button
                              onClick={() => enhanceAchievements(index, 'rewrite')}
                              disabled={!!aiLoadingByExperience[work.id] || work.achievements.length === 0}
                          ...
                          {aiErrorByExperience[work.id] && (
                            <p className="text-xs text-red-600">{aiErrorByExperience[work.id]}</p>
                          )}
                        </div>
                        {work.achievements.map((ach, achIndex) => (
                          <div key={achIndex} className="flex items-center justify-between rounded-lg bg-white p-2 border border-gray-200">
        ...
        {activeTab === 'leadership' && (
          <div className="space-y-4">
            {formData.leadershipCommunity.map((lead, index) => (
              <div key={lead.id} className="rounded-lg border border-gray-200 bg-gray-50">
                <div
                  onClick={() => setExpandedLeadership(expandedLeadership === lead.id ? null : lead.id)}
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-100 cursor-pointer"
                >
                  <div className="text-left">
                    <p className="font-medium text-gray-900">{lead.position || 'Position'}</p>
                    <p className="text-sm text-gray-600">{lead.organization || 'Organization'}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={e => {
                    ...
                    {expandedLeadership === lead.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                  </div>
                </div>
                    ...
                    <div>
                      <label className="flex items-center gap-2">
                        <input
                          ...
                          onChange={e => updateLeadership(index, 'currentlyHolding', e.target.checked)}
                          className="rounded"
                        />
                        <span className="text-sm font-medium text-gray-700">Currently holding this position</span>
                      </label>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Contributions</label>
                      <div className="space-y-2">
                        <div className="flex gap-2">
                          <input
                            type="text"
                            ...
                            onKeyPress={e => {
                              if (e.key === 'Enter') {
                                addLeadershipAchievement(index, (e.target as HTMLInputElement).value);
                                (e.target as HTMLInputElement).value = '';
                              }
                            }}
                            className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none"
                        ...
                        {lead.achievements.map((ach, achIndex) => (
                          <div key={achIndex} className="flex items-center justify-between rounded-lg bg-white p-2 border border-gray-200">
                            <span className="text-sm">● {ach}</span>
                            <button
                              onClick={() => removeLeadershipAchievement(index, achIndex)}
                              className="text-red-500 hover:text-red-700"
                            >
                              ✕
                            </button>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  onKeyPress={e => {
                    if (e.key === 'Enter') {
                      addSkill('softSkills', (e.target as HTMLInputElement).value);
                      (e.target as HTMLInputElement).value = '';
                    }
                  }}
                  className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none"
              ...
              {formData.skills.softSkills.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {formData.skills.softSkills.map(skill => (
                    <div key={skill} className="flex items-center gap-2 rounded-lg bg-blue-100 px-3 py-1 text-sm text-blue-900">
                      {skill}
                      <button
              ...
              {formData.skills.languages.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {formData.skills.languages.map(skill => (
                    <div key={skill} className="flex items-center gap-2 rounded-lg bg-green-100 px-3 py-1 text-sm text-green-900">
                      {skill}
                      <button
            ...
            <div>
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  onKeyPress={e => {
                    if (e.key === 'Enter') {
                      addSkill('programming', (e.target as HTMLInputElement).value);
                      (e.target as HTMLInputElement).value = '';
                    }
                  }}
                  className="flex-1 rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none"
              ...
              {formData.skills.programming.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {formData.skills.programming.map(skill => (
                    <div key={skill} className="flex items-center gap-2 rounded-lg bg-purple-100 px-3 py-1 text-sm text-purple-900">
                      {skill}
                      <button
        ...
        {activeTab === 'honors' && (
          <div className="space-y-4">
            {formData.honors.map((honor, index) => (
              <div key={honor.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-200 bg-gray-50">
