  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-8 text-gray-900 font-serif text-sm leading-relaxed h-full overflow-y-auto">
          ...
          {data.education.map((edu, index) => (
            <div key={index} className="mb-4">
              <div className="flex justify-between items-start">
                ...
                {edu.graduationDate && <p className="text-xs text-gray-600">{edu.graduationDate}</p>}
              </div>
      ...
      {data.experience.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Experience</h2>
          <div className="mt-2 mb-2 h-px bg-gray-800" />
      ...
      {data.leadershipCommunity.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Leadership & Community Involvement</h2>
          <div className="mt-2 mb-2 h-px bg-gray-800" />
          {data.leadershipCommunity.map((lead, index) => (
            <div key={index} className="mb-4">
              <div className="flex justify-between items-start">
                <p className="text-xs italic">{lead.organization}</p>
                {lead.location && <p className="text-xs text-gray-600">{lead.location}</p>}
              </div>
              <div className="flex justify-between items-start mt-1">
                <p className="font-bold text-xs">{lead.position}</p>
                <p className="text-xs text-gray-600">
                  {lead.startDate}
                  {lead.currentlyHolding ? ' - Present' : lead.endDate ? ` - ${lead.endDate}` : ''}
                </p>
              </div>
      ...
      {/* Skills */}
      {(data.skills.softSkills.length > 0 || data.skills.languages.length > 0 || data.skills.programming.length > 0) && (
        <div className="mb-6">
          ...
          <div className="text-xs space-y-2">
            {data.skills.softSkills.length > 0 && (
              <div>
                <span className="font-semibold">Soft Skills:</span> {data.skills.softSkills.join(', ')}
              </div>
            )}
            {data.skills.languages.length > 0 && (
              <div>
                <span className="font-semibold">Languages:</span> {data.skills.languages.join(', ')}
              </div>
            )}
            {data.skills.programming.length > 0 && (
              <div>
                <span className="font-semibold">Programming:</span> {data.skills.programming.join(', ')}
              </div>
            )}
          </div>
        </div>
      ...
      {data.honors.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Honors and Awards</h2>
          <div className="mt-2 mb-2 h-px bg-gray-800" />
          <div className="text-xs space-y-1">
            {data.honors.map((honor, index) => (
              <div key={index} className="flex justify-between">
                <span>{honor.title}</span>
                {honor.year && <span className="text-gray-600">{honor.year}</span>}
              </div>
            ))}
          </div>
        </div>
